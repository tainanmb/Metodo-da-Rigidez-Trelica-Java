...
/**
 * Classe para resolução de sistemas lineares via eliminação de Gauss com
 * sistema matricial da forma Ax = b.
 * <p>
 * Dados:
 * <ul>
...
public abstract class LinearEquationSystem {
	...
	/**
	 * Método de solução de sistemas de equações lineares a partir do método de
	 * Gauss
	 * 
	...
	public static Vetor SolverEquation(Matriz A, Vetor b) {
		...
		// escalonar para k igual 1 (promera linha) até n-1 numero de linhas menos um
		// (penultima linha)
		...
		for (int k = 0; k < A.getN_Linhas() - 1; k++) {
			...
			// para i variando de k+1 (linha abaixo do pivô) até i<número de linhas (ultima
			// linha)
			for (int i = k + 1; i < A.getN_Linhas(); i++) {

				// verifica se os coeficiente do pivo são inferiores
				// aos coeficientes abaixo na posição ik (i: linha abaixo do pivo, k: linha do
				// pivo)
				// caso positivo pivo se torna o coeficiente encontrado e pivoIndex a linha do
				// pivô
			...
			if (pivoIndex != k) {
				// se o pivô foi alterado, inverte as linhas para que ele fique na linha mais
				// alta
			...
			if (A.getMatriz(k, k) == 0) {
				...
				for (int m = k + 1; m < A.getN_Linhas(); m++) {
					...
					// considera que deu certo a linha abaixo do pivô seta na linha m coluna k o
					// zero
					A.setMatriz(m, k, 0);
